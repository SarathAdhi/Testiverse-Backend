...
const protectedRoute = async (req, res, next) => {
    ...
    try {
        ...
        const cachedUser = await redis_1.default.get(String(decoded.user._id));
        if (cachedUser) {
            let user = JSON.parse(cachedUser);
            user = { ...user, _id: new mongoose_1.Types.ObjectId(user._id) };
            req.user = user;
        ...
        await redis_1.default.set(String(decoded.user._id), JSON.stringify(user), "EX", 60 * 60 * 24);
        req.user = user;
    }
    catch (error) {
        req.logout({}, () => { });
